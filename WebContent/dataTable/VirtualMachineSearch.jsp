...
<title>虚拟机交换机监控数据查询</title>
</head>
<body>


<%
	String url = request.getContextPath() + "/dataTable/VirtualMachineSearch.jsp";
%>
...
<%
	...
	ResponseGetRawData data = RequestData.requestRawData(param,RequestEnum.Request_Search_Openvswitch.getValue());
	if(data!=null){
...
<form action=<%=url %> method="post" name="form2">
  ...
  <table>
       <tr>
       <% String urlPaging = request.getContextPath() + "/dataTable/VirtualMachineSearch.jsp?search="+search+"&showPage="; %>
      <%if(showPage>1){ %>
       <td width=150>
